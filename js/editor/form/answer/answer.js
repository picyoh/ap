...
export const addAnswer = (parent, lastAnswers, targetId) => {
  ...
  const questionNumber = rowLevel
    ? 0
    : parseInt(lastAnswers.parentNode.id.split("_")[1]);
  ...
  const questionParentId =
    "question_parent_" + rowNumber + "." + questionNumber;
  ...
  const answer = `
    <div class='answer'>
        ...
        ${
          rowLevel
            ? ``
            : `<div id='answer_link_${number}' class='link' linkable='true' link-target='true'></div>`
        }    
